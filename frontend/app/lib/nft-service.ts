...
export const getCachedNFTData = unstable_cache(
  async (): Promise<RealNFTData[]> => {
    ...
    for (const tokenId of tokenIds) {
      try {
        ...
        const response = await fetch(
          `${baseUrl}/api/nft-data?tokenId=${tokenId}&contractAddress=${CONTRACT_ADDRESS}`,
          {
    ...
    // 确保我们总是有数据返回
    console.log(`Successfully fetched ${nftData.length} NFTs from contract`);
